...
class HelloWorld(MessagingHandler):
    ...
    def on_start(self, event):
        ...
        self.receiver = event.container.create_receiver(conn
                                                        , "amq.topic/the_producer.crawling_list", options=AtMostOnce()
                                                        )
                                                        #, options=Selector("basic-labor = 'yes' AND premium-labor = 'yes'"))
                                                        #, options=Selector("colour = 'gree' AND colour1 = 'gree1'"))
